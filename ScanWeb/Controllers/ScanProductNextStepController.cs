...
{
    public class ScanProductNextStepController : BaseController
    {
        [CommonAuthen]
        public ActionResult Index()
        {
            if (!LoginUser.Current.ISUSERTREAM)
                Response.Redirect("/");
            var resultCheckUser = APIHelper.CallAPI<JsonResultData>("scan/checktrimtableuser", new
            {
            ...
            ViewBag.Table = LoginUser.Current.TREAMNAME;
            return View();
        }
        ...
        public JsonResult Scan(string QRcode)
        {
            try
            {
                var result = APIHelper.CallAPI<JsonResultData>("scan/scanproductionstep2", new
                {
                    PalletCode = QRcode,
