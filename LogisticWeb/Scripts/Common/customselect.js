        ...
        success: function (res) {
            ...
            if (res.IsOk) {
                ...
                if (data != null) {
                    ...
                    var $optionList = [];
                    var noneoption = $item.attr('data-none');
                    if (noneoption == 'true') {
                        var $optionnone = $('<option value="">---Select Options---</option>');
                        $optionList.push($optionnone);
                    }
            ...
            if ($item.attr('afterload') != undefined && $item.attr('afterload') != '') {
                stringFnName = $item.attr('afterload');
                //setTimeout(function () {
                window[stringFnName]($item);
                //}, 300);
