...
namespace LogisticWeb.Controllers
{
    public class ContainerTypeController : BaseController
    {
        ...
        public JsonResult GetList(string pageindex, string pagesize, string filterquery = "", string orderby="")
        {
            ...
            {
                var resultJson = APIHelper.CallAPI<JsonResultObject<List<MD_CONTAINERTYPE>>>("containertype/getlist", new
                {
                    PageIndex = PageIndex,
                    PageSize = PageSize,
        ...
        {
            MD_CONTAINERTYPE model = new MD_CONTAINERTYPE();
            if (!string.IsNullOrEmpty(id))
            {
                var result = APIHelper.CallAPI<JsonResultObject<List<MD_CONTAINERTYPE>>>("containertype/getdetail", new
                {
                    id = id
                });
                if (result.IsOk)
        ...
        public JsonResult Getbyid(string id)
        {
            try
            {
                var resultJson = APIHelper.CallAPI<JsonResultObject<List<MD_CONTAINERTYPE>>>("consignee/getdetail", new
                {
                    id = id
                });
                if (!resultJson.IsOk)
